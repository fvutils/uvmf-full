__all__ = ['version','parent_parser','resolve_path']
...
def resolve_path(input_path,root,input_file=None):
  ...
  if re.match(r'\$',p):
    logging.error("Unresolved environment variables in provided path: {}".format(p))
    if input_file:
